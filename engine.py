...
With Context Pre-Warming, Perceptual Word-Level Consistency & Crackle-Free Resonance
"""
...
class VocalDynamicsEngine:
    def __init__(self, config=None, progress=None):
        ...
        self._y_k_cache = None

    # ─── HELPER: TIME CONSTANT ────────────────────────────────────────────────

    def _time_const(self, ms, sr):
        """Convert milliseconds to one-pole IIR coefficient (frame-domain)."""
        frame_rate = sr / self.cfg.hop
        return 1.0 - np.exp(-1.0 / (frame_rate * max(ms, 1e-6) / 1000.0))

    def _time_const_samples(self, ms, sr):
        """Convert milliseconds to one-pole IIR coefficient (sample-domain)."""
        return 1.0 - np.exp(-1.0 / (sr * max(ms, 1e-6) / 1000.0))

    # ─── HELPER: CONTEXT PRE-WARMING (FIXES FADE-IN ARTIFACT) ─────────────────

    def _prewarm_context(self, env_db, sr):
        """
        Pre-warm multi-scale context with first voiced region level.
        Eliminates fade-in artifact when audio starts from silence by
        preventing contexts from starting at -inf dBFS.
        """
        hop = self.cfg.hop
        n = len(env_db)

        # Find first voiced region (above noise floor)
        noise_floor = np.percentile(env_db, 5)
        voice_threshold = noise_floor + 10
        voiced_mask = env_db > voice_threshold

        if not np.any(voiced_mask):
            return env_db  # All silence, nothing to prewarm

        first_voice_idx = np.argmax(voiced_mask)
        # Use median of first 200ms of voiced content as warm-up level
        warm_end = min(first_voice_idx + int(0.2 * sr / hop), n)
        warm_level = np.median(env_db[first_voice_idx:warm_end])

        # Replace silent prefix with warm-up level
        prewarm = env_db.copy()
        prewarm[:first_voice_idx] = warm_level
    ...
    def process(self, in_path, out_path=None):
        ...
        # Compute K-weighted signal once, cache for downstream use
        ...
        # De-esser on sibilance band only
        ...
        # True-peak limit BEFORE LUFS normalization
    ...
    def _deesser(self, y, sr):
        """Broadband de-esser applied to the sibilance band with pre-warmed context."""
        c = self.cfg.de_esser
        nyq = sr / 2.0
        ...
        # LP-filtered RMS envelope (no Hilbert ringing)
        sos_env = signal.butter(2, 400.0 / nyq, btype='low', output='sos')
        env = np.sqrt(np.maximum(0.0, signal.sosfiltfilt(sos_env, y_sc ** 2)))
        env_db = librosa.amplitude_to_db(env + 1e-10)

        # PRE-WARM CONTEXT to prevent fade-in artifact
        env_db = self._prewarm_context(env_db, sr)
    ...
    def _split_bands_lr4(self, y, sr):
        """True LR4 cascade via successive HP/LP on residual."""
    ...
    def _resonance_suppress(self, y, sr):
        """Crackle-free resonance suppression with sosfiltfilt + Welch PSD."""
    ...
    def _process_bands(self, bands, sr):
        """Per-band dynamics with pre-warmed context."""
        out = []
        gr_dict = {}
        hop = self.cfg.hop
        ...
        for i, name in enumerate(self.cfg.band_names):
            ...
            # PRE-WARM CONTEXT to prevent fade-in artifact
            rms_db = self._prewarm_context(rms_db, sr)
    ...
    def _excite(self, y, sr, amount):
        """Level-dependent harmonic exciter with normalized drive."""
        c = self.cfg.exciter
        hop = self.cfg.hop
    ...
    def _ms_glue(self, y, sr):
        """Psychoacoustic soft-knee compressor with pre-warmed context."""
        c = self.cfg.psycho
        hop = self.cfg.hop
